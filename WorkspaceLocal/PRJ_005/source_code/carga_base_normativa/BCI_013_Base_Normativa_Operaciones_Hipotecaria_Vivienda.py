...
# MAGIC %md
# MAGIC # BCI_013_Base_Normativa_Operaciones_Hipotecaria_Vivienda
...
# MAGIC ### Encabezado
# MAGIC **************************************************************************
# MAGIC * Nombre: BCI_013_Base_Normativa_Operaciones_Hipotecaria_Vivienda.ipynb
# MAGIC * Ruta: 
...
# MAGIC * base_archivos_normativos (Activo 21 Tabla 89 Manual CMF)
...
dbutils.widgets.text("tipo_modelo_hipW","","07 Tipo Modelo Hipotecario :")
...
tipo_modelo_hipX = dbutils.widgets.get("tipo_modelo_hipW")
spark.conf.set("bci.tipo_modelo_hipX", tipo_modelo_hipX)
...
print("tipo_modelo_hipX: "+ tipo_modelo_hipX)
...
# MAGIC %md
# MAGIC ### Tabla tmp_prestamo_hip_vivienda

# COMMAND ----------

paso_tb_del_tmp_prestamo_hip_interno  = """DROP TABLE IF EXISTS """ + db_plat_tempX + """.tmp_prestamo_hip_vivienda"""

# COMMAND ----------

sql_safe(paso_tb_del_tmp_prestamo_hip_interno)

# COMMAND ----------

dbutils.fs.rm(db_location_plat_tempX+"tmp_prestamo_hip_vivienda", True)

# COMMAND ----------

paso_tb_crea_tmp_prestamo_hip_interno  = """CREATE TABLE """ + db_plat_tempX + """.tmp_prestamo_hip_vivienda
(
...
LOCATION '"""+ db_location_plat_tempX +"""tmp_prestamo_hip_vivienda' """

# COMMAND ----------

sql_safe(paso_tb_crea_tmp_prestamo_hip_interno)
...
# MAGIC %md
# MAGIC ###Paso 1: Obtencion Operaciones Hipotecarias Modelo Interno

# COMMAND ----------

# MAGIC %md
# MAGIC se crea una vista del universo de operaciones hipotecarias para la vivienda  utilizando tablas tmp_d00_deuda_act_ctg (Salida de Notebook 1)y tabla TBU para identificar cuenta Ifrs
# MAGIC
# MAGIC **Colocaciones Comerciales Operaciones Vivienda**
# MAGIC - **146000100**: Préstamos con letras de crédito para vivienda (Colocaciones para vivienda)
# MAGIC - **146000200**: Préstamos con mutuos hipotecarios endosables (Colocaciones para vivienda)
# MAGIC - **145400104**: Préstamos con letras de crédito para fines generales (Préstamos comerciales)
# MAGIC - **146000400**: Otros créditos con mutuos para vivienda
# MAGIC - **146000903**: Créditos provenientes de la ANAP (Otros créditos y cuentas por cobrar)
# MAGIC - **146000901**: Créditos complementarios a los mutuos (Otros créditos y cuentas por cobrar)
# MAGIC - **146000902**: Créditos de enlace (Otros créditos y cuentas por cobrar)
# MAGIC - **146000909**: Cuentas por cobrar a deudores para vivienda
# MAGIC
...
query_ins_hip_int = """ insert into """+db_plat_tempX+""".tmp_prestamo_hip_vivienda
select d00.periodo_id
  ...
  ,d00.fl_ope_reneg 
	,21 as cod_tipo_activo
	,'HIPOTECARIOS' as nombre_activo 
	,0 as pct_pi
	,0 as pct_pdi
	,0 as pct_pe
	,0 as mto_provision
...
where d00.periodo_id =  """+anomes+"""
	and trim(d00.cod_cartera_ope) = 'ACT'
 	and ccc.cta_cmf IN ('146000100','145400104','146000200', '146000402','146000903','146000901', '146000902','146000909')
  and ccc.fecha_informada = """+str(dia_tbu)+"""
	--Otros créditos con mutuos para vivienda puede ser 146000400 o 146000402
 --validar si debe sumar item Otros créditos con mutuos no endosables para vivienda
  /* IN ('1304101','1304102','1304200',
							 '1304500','1304600','1304901',
							 '1304911','1304981') */
 """

# COMMAND ----------

sql_safe(query_ins_hip_int)

# COMMAND ----------

# MAGIC %md
# MAGIC ###Paso 2: Eliminación Creditos Hipotecarios para la Vivienda por Tipo de Activo para fecha de Proceso

# COMMAND ----------

query_delete_hip_int = """DELETE FROM """ + db_platinumX + """.base_archivos_normativos WHERE periodo_id = """+anomes+""" and cod_tabla_89 in (21) and trim(cod_cartera_ope) = 'ACT' """

# COMMAND ----------

sql_safe(query_delete_hip_int)

# COMMAND ----------

# MAGIC %md
# MAGIC ###Paso 3: Insert Operaciones Hipotecarias Vivienda Modelo Interno

# COMMAND ----------

# MAGIC %md
# MAGIC Se insertan operaciones Estudiantiles Activas con Creditos Estudiantiles utilizando tablas tmp_prestamo_estudiantil generada en Paso 1y tmp_modelo_hip, Tabla de Salida de Notebook 4 respectivamente
# MAGIC
...
query_ins_hip_interno = """ insert into """ + db_platinumX + """.base_archivos_normativos
select a.periodo_id
	,a.rut_cliente
  ,a.dv_cliente
	,a.cod_ope_original
  ...
  ,b.pct_pdi
  ,b.pct_pe
	,b.mnt_provision as mto_provision
	,a.cod_tipo_cli
	,a.mnt_exposicion
	,a.factor_expo
	,a.des_tipo_gtia as TIPO_GARANTIA				
	,a.mnt_garantia as MONTO_GARANTIA
	,b.pct_ltv as PTVG 
	,a.pct_pi_metodo_interno as pct_pi_metodo_interno
  ,a.pct_pdi_metodo_interno as  pct_pdi_metodo_interno
  ,a.pct_pe_metodo_interno  as pct_pe_metodo_interno
	,a.mnt_prov_metodo_interno as mto_PROV_METODO_INTERNO 
	,a.mnt_avalado
 ,'"""+str(date_proceso)+"""'
from """+db_plat_tempX+""".tmp_prestamo_hip_vivienda  a
inner join """+db_plat_tempX+""".tmp_modelo_hip b
...
if(getArgument("tipo_modelo_hipW") == 'INTERNO'):
  sql_safe(query_ins_hip_interno)

# COMMAND ----------

# MAGIC %md
# MAGIC ###Paso 4: Insert Operaciones Hipotecarias Vivienda Modelo Estanda (B1)

# COMMAND ----------

# MAGIC %md
# MAGIC Se insertan operaciones Hipotecarias Viviemda Modelo Estandar tablas tmp_prestamo_hip_vivienda generada en Paso 1y tmp_modelo_hip_estandar, Tabla de Salida de Notebook4 respectivamente
# MAGIC
# MAGIC **Tabla de salida**: base_normativa_v3_ft
# MAGIC
# MAGIC **variables de salida:**
...
select a.periodo_id
  ...
  ,a.pct_pe_metodo_interno  as pct_pe_metodo_interno
	...
	,a.mnt_avalado
 	,'"""+str(date_proceso)+"""'
from """+db_plat_tempX+""".tmp_prestamo_hip_vivienda a
inner join """+db_plat_tempX+""".tmp_modelo_hip_estandar b
  on a.periodo_id = b.periodo_id
  and trim(a.cod_cartera_ope) = 'ACT'
  and trim(a.cod_num_operacion) = trim(b.cod_num_operacion)
where a.periodo_id =  """+anomes+"""
...
if(getArgument("tipo_modelo_hipW") == 'ESTANDAR'):
...
registros = spark.sql(""" select count(1) as cant_registros from """ + db_platinumX + """.base_archivos_normativos  where periodo_id = """+anomes+""" and cod_tabla_89 = 21 """)
